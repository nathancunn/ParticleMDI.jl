...
"""
function parallel_pmdi(dataFiles, dataTypes, N::Int64, particles::Int64,
    ρ::Float64, iter::Int64, outputFile::String, initialise::Bool)
    ...
    # Mutation weights
    logprob = zeros(Float64, N)
    fprob = zeros(Float64, N)


    # Initialise the particles
    particle = [Vector{dataTypes[k]}(particles) for k = 1:K]
    particle_IDs = ones(Matrix{Int64}(particles, K), Int64)
    new_particle_IDs = ones(Matrix{Int64}(particles, K), Int64)

    for k = 1:K
        particle[k][1] = dataTypes[k](dataFiles[k], N)
    end
    # Calculate likelihood
    ll = calculate_likelihood(s, Φ, γ, Z)

    # Save data titles to output file
    ...
    for it in 1:iter
        ...
        # Fix ρ of the observations to same value from before
        ...
        for i in order_obs[(floor(Int64, ρ * n_obs) + 1):n_obs]
            ancestor_weights .= logweight .+ 0
             for k = 1:K
                @inbounds obs = dataFiles[k][i, :]
                for p in 1:maximum(particle_IDs[:, k])
                    @inbounds p_flag = findindices(particle_IDs[:, k], p)
                    particleMDI.calc_logprob!(logprob, obs, particle[k][p])
                    (sstar[p_flag, k], logweight[p_flag]) = draw_sstar_p!(sstar[p_flag, k], logprob,
                                  Π, N, ancestor_weights[p_flag], logweight[p_flag], s[i, k])
                end

                sstar[1, k] = s[i, k]
                new_particle_IDs[:, k] = update_particleIDs(particle_IDs[:, k], sstar[:, k], particles, N)
                @inbounds id_match = ID_match(particle_IDs[:, k], new_particle_IDs[:, k], particles)
                # Get new particle IDs
                # Allows to only do the particle updating where necessary
                ...
                particle_IDs[:, k] = new_particle_IDs[:, k]
            end
            ...
            # new_particle_IDs = update_particleIDs(particle_IDs, sstar, particles, N)
            ...
            if calc_ESS(logweight) <= 0.5 * particles
                ...
                sstar .= Int64(1)
                particle_IDs = update_particleIDs(particle_IDs, 1, particles, N)
            end
        end
