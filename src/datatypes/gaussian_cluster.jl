...
function calc_logprob!(logprob, obs::Array, cl::particleMDI.GaussianCluster)
  logprob[1] = - 1.310533
   for obs_q in obs
  ...
  for i in 1:length(logprob)
      if cl.n[i] > 0
          ...
          @fastmath @inbounds logprob[i] = Q * (Base.Math.JuliaLibm.lgamma(n + 0.5) -
...
function particle_copy!(out::particleMDI.GaussianCluster, particle::particleMDI.GaussianCluster)
